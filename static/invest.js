...
function updateLoanAmount() {
  ...
  // math for Loan Total
  const investmentTotal = parseInt(trancheAmt) * parseInt(numTranches);
  document.getElementById('investmentTotal').value = Number(investmentTotal).toLocaleString();
}
...
document.getElementById('investmentTotal').addEventListener('change', updateLoanAmount);

const investmentTotalInput = document.getElementById('investmentTotal');
...
// Function to calculate loan total and update investmentTotalInput
function updateinvestmentTotal() {
  // Calculate loan total
  const investmentTotal = trancheAmt * parseInt(numTranchesInput.value);
  
  // Update loan total input field with formatted loan total value
  investmentTotalInput.value = Number(investmentTotal).toLocaleString();
}
...
function updateNumTranches() {
  ...
  const numTranches = parseInt(investmentTotalInput.value.replace(",", "")) / trancheAmt;
...
investmentTotalInput.addEventListener('input', function() {
  // Check if the investmentTotalInput input field has a value
  if (investmentTotalInput.value) {
    const quotient = parseInt(investmentTotalInput.value.replace(",", "")) / trancheAmt;
    if (Number.isInteger(quotient)) {
      // Update numTranches based on investmentTotal/trancheAmt
      updateNumTranches();
    }
  } else {
...
numTranchesInput.addEventListener('input', function() {
  ...
  if (numTranchesInput.value) {
    // Update investmentTotal based on trancheAmt*numTranches
    updateinvestmentTotal();
  } else {
    // Clear investmentTotal if numTranchesInput is empty
    investmentTotalInput.value = '';
  }
});
...
function handleInvestorSelect() {
  var select = document.getElementById("investorDropdown");
